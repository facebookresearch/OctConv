...
def main():
    ...
    kwargs = {'ctx': context, 'pretrained': opt.use_pretrained, 'classes': classes,
              'ratio': opt.ratio, 'final_drop': opt.final_drop}
    if opt.last_gamma:
        ...
        for epoch in range(opt.resume_epoch, opt.num_epochs):
            ...
            for i, batch in enumerate(train_data):
                ...
                if epoch == 0 and first_fwd and opt.mode == 'hybrid':
                    net.export("/tmp/net")
                    mx.visualization.print_summary(mx.symbol.load('/tmp/net-symbol.json'), 
                            shape={'data': (batch_size, 3, opt.input_size, opt.input_size)})
                    first_fwd = False
